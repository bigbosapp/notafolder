...
function sysSanitizeDB() {
    ...
    storage.forEach(item => {
        if (item.parentId && item.parentId !== 'HOME') {
            ...
            if (!parentExists) { item.parentId = null; changed = true; }
        }
    });
    if(changed) dbSave();
}
...
function sysAutoCleanup() {
    ...
    storage = storage.filter(item => { if (item.inTrash && item.deletedAt) { const age = now - new Date(item.deletedAt).getTime(); return age < TWO_DAYS_MS; } return true; });
    const initialHisLen = moveHis.length;
    moveHis = moveHis.filter(h => { const age = now - (h.timestamp || now); return age < TWO_DAYS_MS; });
    if (storage.length !== initialStorageLen || moveHis.length !== initialHisLen) { dbSave(); }
}

function getUniqueName(name, parentId, type) {
    let newName = name; let counter = 1; let safeGuard = 0; 
    while (storage.some(i => i.parentId === parentId && i.type === type && i.name.toLowerCase() === newName.toLowerCase() && !i.inTrash) && safeGuard < 500) { newName = `${name} (${counter})`; counter++; safeGuard++; }
    return newName;
}
...
function sysNum(val) { if (!val) return 0; const clean = val.toString().replace(/\./g, '').replace(',', '.'); return parseFloat(clean) || 0; }

/* --- UI SYSTEM --- */
function uiShowChangelog() {
    const logs = [
        "<b>v121.16</b>: Fix Desktop Sort Button & Back Button Guard.",
        "<b>v121.15</b>: Icon Sort Baru & Header Klasik.",
        "<b>v121.14</b>: Header Rapi & Fitur Sorting.",
        "<b>v121.12</b>: Fix Crash Move & Rename Case."
    ];
...
function sysAppResetConfirm() {
    uiPopupReset();
    const p = document.getElementById('comp-popup'); const btn = document.getElementById('btn-popup-confirm'); const bc = document.getElementById('btn-popup-cancel'); const desc = document.getElementById('popup-desc');
    p.classList.remove('hidden'); document.getElementById('popup-icon').innerText = '☢️'; document.getElementById('popup-title').innerText = 'Reset Aplikasi';
    desc.innerHTML = `<b>PERHATIAN!</b><br>Seluruh data akan <b>DIHAPUS PERMANEN</b>.<br>Pastikan Anda sudah Export data.`; desc.classList.remove('hidden');
    let count = 5; btn.disabled = true; btn.style.background = "#94a3b8"; btn.innerText = `Tunggu ${count}s`;
    resetTimer = setInterval(() => { count--; if(count > 0) { btn.innerText = `Tunggu ${count}s`; } else { clearInterval(resetTimer); btn.disabled = false; btn.style.background = "var(--success)"; btn.innerText = "YA, HAPUS SEMUA"; btn.onclick = () => { localStorage.clear(); location.reload(); }; } }, 1000);
    bc.onclick = () => { clearInterval(resetTimer); uiPopupClose(); };
}

/* --- CALCULATION LOGIC --- */
function sysGetFolderTotal(parentId) {
    let total = 0; const children = storage.filter(i => i.parentId === parentId && !i.inTrash);
    children.forEach(child => { if (child.type === 'nota' && child.items) { child.items.forEach(item => { total += (sysNum(item.jml) * sysNum(item.harga)); }); } else if (child.type === 'folder') { total += sysGetFolderTotal(child.id); } });
    return total;
}
...
function sysSumSelected() {
    ...
    selIds.forEach(id => {
        ...
        if(item) { let itemVal = 0; if(item.type === 'nota' && item.items) { item.items.forEach(r => itemVal += (sysNum(r.jml) * sysNum(r.harga))); files.push({ name: item.name, val: itemVal }); } else if(item.type === 'folder') { itemVal = sysGetFolderTotal(item.id); folders.push({ name: item.name, val: itemVal }); } totalSum += itemVal; }
    });
...
function sysToggleSelectAll(isChecked) {
    ...
    document.getElementById('txt-select-count').innerText = `${selIds.length} Item`; navRenderGrid();
}
...
function sysUpdateBreadcrumbs() {
    ...
    let pathStack = []; let tempParent = curParent; if (activeId) { const n = storage.find(i => i.id === activeId); if(n) tempParent = n.parentId; }
    ...
    pathStack.forEach(f => { let li = document.createElement('li'); li.innerText = escapeHtml(f.name); li.onclick = () => navGo(f.id); bc.appendChild(li); });
    if (activeId) { const currentFile = storage.find(i => i.id === activeId); if (currentFile) { let li = document.createElement('li'); li.innerText = escapeHtml(currentFile.name); li.className = 'active-file'; bc.appendChild(li); } }
}

function sysQuickPreview(id) { activeId = id; history.pushState({view: 'preview', id: id}, null, ""); uiPreview(true, true); }

function navRenderGrid() {
    document.body.classList.remove('mode-editor'); document.getElementById('view-editor').classList.add('hidden'); document.getElementById('view-list').classList.remove('hidden'); document.getElementById('select-bar').classList.toggle('hidden', !selMode);
    sysUpdateBreadcrumbs();
    const grid = document.getElementById('comp-grid'); grid.innerHTML = '';
    document.getElementById('vm-simple').classList.toggle('active', viewMode === 'simple'); document.getElementById('vm-details').classList.toggle('active', viewMode === 'details');
    ...
    items.forEach(item => {
        ...
        let dispName = escapeHtml(item.name);
        const limit = viewMode === 'simple' ? 13 : 18; if(dispName.length > limit) dispName = dispName.substring(0, limit) + '...';
        ...
        if (viewMode === 'simple') {
            ...
            const icon = item.type === 'folder' ? '📁' : '📄'; const dateEdit = formatDateDetail(item.lastEdited); let metaHtml = '';
            if (item.type === 'folder') { const count = storage.filter(x => x.parentId === item.id && !x.inTrash).length; const folderTotal = sysGetFolderTotal(item.id); metaHtml = `<div class="meta-line">📂 Isi: ${count} File</div><div class="meta-line" style="color:#16a34a;font-weight:bold;">💰 Rp ${folderTotal.toLocaleString('id-ID')}</div><div class="meta-line">✏️ Edit: ${dateEdit}</div>`; } 
            else { let itemCount = item.items ? item.items.length : 0; let total = 0; if(item.items) item.items.forEach(r => total += (sysNum(r.jml) * sysNum(r.harga))); metaHtml = `<div class="meta-line">📦 ${itemCount} Barang</div><div class="meta-line" style="color:var(--nota);font-weight:bold;">💰 Rp ${total.toLocaleString('id-ID')}</div><div class="meta-line">✏️ Edit: ${dateEdit}</div>`; }
...
// --- PERBAIKAN: Fungsi sysSortChange hanya mengganti class ---
function sysSortChange(type, val) {
    if(type === 'p') {
        sortPrio = val; localStorage.setItem('notafolder_sort_prio', sortPrio);
        document.querySelectorAll('.sort-col:nth-child(1) .sort-btn').forEach(b => b.classList.remove('active'));
        document.getElementById('s-p-' + val).classList.add('active');
    } else {
        sortOrder = val; localStorage.setItem('notafolder_sort_order', sortOrder);
        document.querySelectorAll('.sort-col:nth-child(2) .sort-btn').forEach(b => b.classList.remove('active'));
        document.getElementById('s-o-' + val).classList.add('active');
    }
    navRenderGrid();
}
...
function sysSearch(k) { 
    ...
    r.forEach(item => {
        ...
        if (viewMode === 'simple') {
            ...
            card.style.borderTop = `5px solid ${item.type === 'folder' ? 'var(--folder)' : 'var(--nota)'}`;
            card.innerHTML = `${qvBtn}<div class="btn-rename-u" onclick="event.stopPropagation(); uiPopupOpen('rename', '${item.id}')">✎</div><div class="card-icon">${item.type==='folder'?'📁':'📄'}</div><b>${escapeHtml(item.name)}</b><span class="time-label">di ${escapeHtml(l)}</span>`;
            card.onclick = () => item.type === 'folder' ? navGo(item.id) : editOpen(item.id);
            grid.appendChild(card);
        } else {
            const row = document.createElement('div'); row.className = 'list-item';
            const icon = item.type === 'folder' ? '📁' : '📄'; let metaHtml = `<div class="time-label">Lokasi: ${escapeHtml(l)}</div>`;
            row.innerHTML = `<div class="list-icon">${icon}</div><div class="list-body"><div class="list-title">${escapeHtml(item.name)}</div><div class="list-meta-row">${metaHtml}</div></div>${qvBtn}<div class="btn-rename-u" onclick="event.stopPropagation(); uiPopupOpen('rename', '${item.id}')">✎</div>`;
...
function navGo(id) { 
    ...
    document.body.classList.remove('mode-editor'); document.getElementById('view-editor').classList.add('hidden'); document.getElementById('view-list').classList.remove('hidden'); document.getElementById('select-bar').classList.add('hidden'); 
...
function navBack(isHardware = false, force = false) { 
    if(!force && document.body.classList.contains('mode-editor')) {
        ...
        if(!statusText.includes("Siap Cetak")) { uiConfirmAction("Data Belum Disimpan!", "Perubahan akan <b>HILANG</b>.<br>Tetap ingin keluar?", () => { navBack(isHardware, true); }, true); return; }
    }
    if(!isHardware) { if(activeId) { const n = storage.find(i => i.id === activeId); navGo(n ? n.parentId : null); } else if(curParent) { const p = storage.find(i => i.id === curParent); navGo(p ? p.parentId : null); } }
}

// --- PERBAIKAN: Block Back Button Hardware/Browser ---
window.onpopstate = function(event) { 
    if(document.body.classList.contains('mode-editor')) {
        ...
        if(!statusText.includes("Siap Cetak")) {
            ...
            uiConfirmAction("Data Belum Disimpan!", "Tombol Back ditekan. Simpan data dulu!<br>Keluar paksa?", () => { editStatus(true); history.back(); }, true);
            return;
        }
    }
    ...
    if(event.state) {
        if(event.state.view === 'editor') {
            ...
            if(n) {
                ...
                document.body.classList.add('mode-editor'); document.getElementById('view-list').classList.add('hidden'); document.getElementById('view-editor').classList.remove('hidden'); document.getElementById('txt-nota-title').innerText = n.name; 
                document.getElementById('chk-show-note').checked = (n.showNote !== undefined ? n.showNote : !!n.noteContent);
                document.getElementById('inp-nota-note').classList.toggle('hidden', !document.getElementById('chk-show-note').checked); document.getElementById('inp-nota-note').value = n.noteContent || "";
                document.getElementById('chk-show-resi').checked = (n.showResi !== undefined ? n.showResi : !!n.resi);
                document.getElementById('inp-resi').classList.toggle('hidden', !document.getElementById('chk-show-resi').checked); document.getElementById('inp-resi').value = n.resi || "";
...
function uiPopupReset() {
    document.getElementById('popup-desc').classList.add('hidden'); document.getElementById('popup-input-wrapper').classList.add('hidden'); document.getElementById('popup-content-calc').classList.add('hidden'); document.getElementById('popup-opt-wrapper').classList.add('hidden'); document.getElementById('popup-disc-wrapper').classList.add('hidden'); document.getElementById('popup-select').classList.add('hidden'); document.getElementById('btn-popup-extra').classList.add('hidden'); document.getElementById('popup-sort-wrapper').classList.add('hidden');
    ...
    btn.classList.remove('hidden'); bc.classList.remove('hidden'); btn.disabled = false; btn.style.background = "var(--nota)"; btn.innerText = "Simpan"; bc.onclick = () => { uiPopupClose(); }; document.getElementById('popup-input').value = ""; document.getElementById('popup-desc').innerHTML = ""; 
}
...
function uiPopupOpen(type, extra = null) {
    ...
    if(type === 'folder' || type === 'nota') { 
        icon.innerText = type === 'folder' ? '📁' : '📄'; title.innerText = "Buat " + type; input.placeholder = "Nama " + type; wrap.classList.remove('hidden'); input.classList.remove('hidden'); sel.classList.add('hidden'); optWrap.classList.remove('hidden'); 
        ...
        icon.innerText = '🏷️'; title.innerText = "Atur Diskon"; wrap.classList.remove('hidden'); input.classList.add('hidden'); discWrap.classList.remove('hidden'); const n = storage.find(i=>i.id===activeId); document.getElementById('disc-type').value = n.discType || 'n'; document.getElementById('disc-val').value = n.discVal || '';
        btn.onclick = () => { const dType = document.getElementById('disc-type').value; const dVal = parseFloat(document.getElementById('disc-val').value) || 0; if(dType === 'p' && dVal > 100) return uiNotify("Maksimal 100%", "danger"); let subtotal = 0; document.querySelectorAll('#comp-tbody tr').forEach(tr => { const rawQ = tr.querySelector('.col-jml').value.replace(/\./g, ''); const rawP = tr.querySelector('.col-harga').value.replace(/\./g, ''); subtotal += ((parseFloat(rawQ)||0) * (parseFloat(rawP)||0)); }); if(dType === 'n' && dVal > subtotal) return uiNotify("Diskon melebihi total!", "danger"); n.discType = dType; n.discVal = dVal; editCalc(); uiPopupClose(); uiNotify("Diskon Disimpan"); };
    }
    else if(type === 'html_export') {
        icon.innerText = '🌐'; title.innerText = "Export HTML"; desc.innerHTML = `Beberapa browser Android tidak dapat melakukan Download. Gunakan Salin HTML untuk menyalin kode.`; desc.style.fontSize = "11px"; desc.style.textAlign = "justify"; desc.classList.remove('hidden'); wrap.classList.add('hidden'); 
        bc.innerText = "Salin HTML"; bc.style.background = "#8b5cf6"; bc.style.color = "white"; bc.onclick = () => { sysCopyHTML(); }; btn.innerText = "Download HTML"; btn.style.background = "var(--success)"; btn.onclick = () => { sysDownloadHTMLAction(); uiPopupClose(); }; extraBtn.classList.remove('hidden'); extraBtn.onclick = () => uiPopupClose();
    }
    else if(type === 'calc') { icon.innerText = '🔢'; title.innerText = "Rincian"; calcArea.classList.remove('hidden'); document.getElementById('popup-total-val').innerText = extra.total.toLocaleString('id-ID'); const listArea = document.getElementById('popup-calc-list'); listArea.innerHTML = ''; extra.folders.forEach(f => { listArea.innerHTML += `<div class="calc-row is-folder"><span style="flex:1"><span class="calc-icon-sm">📁</span> ${f.name}</span><span>Rp ${f.val.toLocaleString('id-ID')}</span></div>`; }); extra.files.forEach(f => { listArea.innerHTML += `<div class="calc-row is-nota"><span style="flex:1"><span class="calc-icon-sm">📄</span> ${f.name}</span><span>Rp ${f.val.toLocaleString('id-ID')}</span></div>`; }); btn.innerText = "Tutup"; btn.style.background = "var(--success)"; bc.classList.add('hidden'); btn.onclick = () => uiPopupClose(); }
    else if(type === 'rename') { 
        const item = storage.find(i => i.id === extra); icon.innerText = '✎'; title.innerText = "Ubah Nama"; wrap.classList.remove('hidden'); input.classList.remove('hidden'); sel.classList.add('hidden'); input.value = item.name; btn.innerText = "Update"; btn.style.background = "var(--success)"; 
        ...
        sel.innerHTML = '<option value="">-- Pilih Folder --</option><option value="HOME">🏠 BERANDA</option>'; validDestinations.sort((a,b)=>a.name.localeCompare(b.name)).forEach(f => { sel.innerHTML += `<option value="${f.id}">📁 ${f.name}</option>`; }); 
    ...
    else if(type === 'col') { icon.innerText = '➕'; title.innerText = "Tambah Variasi"; wrap.classList.remove('hidden'); input.classList.remove('hidden'); sel.classList.add('hidden'); input.placeholder = "Nama Variasi"; btn.innerText = "Tambah"; btn.style.background = "var(--nota)"; btn.onclick = () => { if(input.value.trim()){ editAddColAction(input.value.trim()); uiPopupClose(); } }; }
    else if(type === 'changelog') { icon.innerText = '📜'; title.innerText = "Riwayat Update"; desc.classList.remove('hidden'); desc.style.textAlign = "left"; desc.style.maxHeight = "300px"; desc.style.overflowY = "auto"; desc.innerHTML = extra.map(l => `<div style="border-bottom:1px solid #eee; padding:8px 0;">${l}</div>`).join(''); btn.innerText = "Tutup"; btn.onclick = () => uiPopupClose(); bc.classList.add('hidden'); }
    else if(type === 'sort') {
        ...
        if(document.getElementById('s-p-'+sortPrio)) document.getElementById('s-p-'+sortPrio).classList.add('active');
        if(document.getElementById('s-o-'+sortOrder)) document.getElementById('s-o-'+sortOrder).classList.add('active');
    }
...
function sysRestore(id) { const item = storage.find(x=>x.id===id); let target = item.parentId; let parentExists = true; if (target) { const parentFolder = storage.find(p => p.id === target && !p.inTrash); if (!parentFolder) parentExists = false; } if(!parentExists) { let newFolder = storage.find(f => f.parentId === null && f.name === "Dipulihkan" && f.type === 'folder' && !f.inTrash); if(!newFolder) { const nfId = 'ID_RECOVER_' + Date.now(); newFolder = { id: nfId, parentId: null, name: "Dipulihkan", type: 'folder', uCode: 'REC', inTrash: false, createdAt: new Date().toISOString(), lastEdited: new Date().toISOString(), items: [] }; storage.push(newFolder); } target = newFolder.id; item.name = getUniqueName(item.name, target, item.type); sysAddHistory(`<b>${item.name}</b> <span class="his-detail">dipulihkan ke</span> "Dipulihkan"`, 'restore'); uiNotify("Folder asli hilang, dipulihkan ke folder baru"); } else { item.name = getUniqueName(item.name, target, item.type); sysAddHistory(`<b>${item.name}</b> <span class="his-detail" style="color:green">dipulihkan</span>`, 'restore'); uiNotify("Dipulihkan!"); } item.parentId = target; item.inTrash = false; dbSave(); uiTrashTab(curTrashTab); navRenderGrid(); }
...
function uiTrashTab(t) { 
    curTrashTab = t; const tabs = { folder: 'tab-f', nota: 'tab-n', history: 'tab-h' }; Object.keys(tabs).forEach(k => { document.getElementById(tabs[k]).className = 'tab-btn' + (k === t ? ' active-' + (t==='history'?'h':t[0]) : ''); }); const list = document.getElementById('trash-list-content'); list.innerHTML = ''; const btnEmpty = document.getElementById('btn-trash-empty'); const filterBar = document.getElementById('history-filter-bar'); 
    if(t === 'history') { btnEmpty.classList.add('hidden'); filterBar.classList.remove('hidden'); renderHistoryList('all'); } else { btnEmpty.classList.remove('hidden'); filterBar.classList.add('hidden'); const now = new Date(); storage.filter(i => i.inTrash && i.type === t).sort((a, b) => new Date(b.deletedAt) - new Date(a.deletedAt)).forEach(i => { const delTime = new Date(i.deletedAt).getTime(); const exp = (delTime + (2 * 24 * 60 * 60 * 1000)) - now; const d = Math.floor(exp/86400000); const h = Math.floor((exp%86400000)/3600000); const m = Math.floor((exp%3600000)/60000); list.innerHTML += `<div style="padding:10px;border-bottom:1px solid #eee;display:flex;justify-content:space-between;align-items:center;"><div style="text-align:left"><b>${escapeHtml(i.name)}</b><br><span class="trash-countdown">🕒 ${d}H ${h}J ${m}M</span></div><button onclick="sysRestore('${i.id}')" style="background:var(--nota);color:white;padding:5px 10px;border-radius:5px;font-size:10px;border:none;cursor:pointer;">Restore</button></div>`; }); } 
}
...
/* --- EDITOR FUNCTIONS --- */
function toggleInput(type) { const chk = document.getElementById(type === 'note' ? 'chk-show-note' : 'chk-show-resi'); const inp = document.getElementById(type === 'note' ? 'inp-nota-note' : 'inp-resi'); inp.classList.toggle('hidden', !chk.checked); if(chk.checked) inp.focus(); editStatus(false); }
function toggleDateSection() { const chk = document.getElementById('chk-show-date'); const wrap = document.getElementById('date-wrapper'); const d1 = document.getElementById('inp-d1'); const d2 = document.getElementById('inp-d2'); editStatus(false); if (chk.checked) { wrap.classList.remove('hidden'); } else { const hasD1 = d1 && d1.value; const hasD2 = d2 && d2.value; if(hasD1 || hasD2) { chk.checked = true; uiConfirmAction("Matikan Tanggal?", "Tanggal yang sudah diisi akan <b>DIHAPUS/RESET</b>.<br>Lanjutkan?", () => { document.getElementById('chk-show-date').checked = false; wrap.classList.add('hidden'); if(d1) d1.value = ""; if(d2) d2.value = ""; editSave(); }, true); } else { wrap.classList.add('hidden'); } } }
...
function editOpen(id) { 
    ...
    const chkNum = document.getElementById('chk-show-num'); if(chkNum) chkNum.checked = (n.showNum !== undefined ? n.showNum : true);
    editCalc(); editStatus(true); if (!history.state || history.state.view !== 'editor' || history.state.id !== id) { history.pushState({view: 'editor', id: id}, null, ""); } localStorage.setItem('notafolder_active_id', id); sysUpdateBreadcrumbs();
}

function sysSetDateMode(m, mark = true) { const n = storage.find(i=>i.id===activeId); n.dateMode = m; document.getElementById('btn-mode-single').classList.toggle('active', m === 'single'); document.getElementById('btn-mode-double').classList.toggle('active', m === 'double'); const b = document.getElementById('comp-date-row'); if (m === 'single') { b.innerHTML = `<div>Tanggal:<input type="date" id="inp-d1" class="edit-inp" value="${n.date1||''}" max="9999-12-31" oninput="editStatus(false)"></div>`; } else { b.innerHTML = `<div style="display: flex; gap: 10px;"><div style="flex:1">Tgl. Masuk:<input type="date" id="inp-d1" class="edit-inp" style="width:100%" value="${n.date1||''}" max="9999-12-31" oninput="editStatus(false)"></div><div style="flex:1">Tgl. Keluar:<input type="date" id="inp-d2" class="edit-inp" style="width:100%" value="${n.date2||''}" max="9999-12-31" oninput="editStatus(false)"></div></div>`; } renderHeader(); if(mark) editStatus(false); }
function renderHeader() { let h = `<tr><th width="30" style="text-align:center;"><input type="checkbox" id="chk-show-num" onchange="editStatus(false)" title="Tampilkan Nomor di Nota"></th><th>Barang</th><th width="60">Jml</th><th width="100">Harga (Rp)</th>`; customCols.forEach((c, i) => h += `<th>${c} <span style="color:red;cursor:pointer" onclick="editRemCol(${i})">x</span></th>`); h += `<th width="40"><div class="btn-add-col-circle" onclick="editAddCol()">+</div></th></tr>`; document.getElementById('comp-thead').innerHTML = h; }
...
function unformatInput(el) { let val = el.value.replace(/\./g, ''); if(val === '0') val = ''; el.value = val; }
function formatInput(el) { let val = el.value.replace(/[^0-9]/g, ''); if(val) { el.value = parseInt(val).toLocaleString('id-ID'); } checkInputLimit(el); editCalc(); }
...
function editAddRow(d = {}) { 
    ...
    tr.innerHTML = `<td style="text-align:center; font-size:12px; color:#64748b; font-weight:bold; vertical-align:middle;">${index}</td><td><textarea class="edit-inp col-barang" rows="1" oninput="checkInputLimit(this); editStatus(false)">${d.barang||''}</textarea></td><td><textarea class="edit-inp col-jml" rows="1" onkeydown="return disableEnter(event)" onfocus="unformatInput(this)" onblur="formatInput(this)" oninput="editCalc(); editStatus(false); checkInputLimit(this)">${d.jml!==undefined?d.jml:''}</textarea></td><td><textarea class="edit-inp col-harga" rows="1" onkeydown="return disableEnter(event)" onfocus="unformatInput(this)" onblur="formatInput(this)" oninput="editCalc(); editStatus(false); checkInputLimit(this)">${d.harga!==undefined?d.harga:''}</textarea></td>${ex}<td style="text-align:center;"><input type="checkbox" class="row-chk"></td>`; 
...
function editDeleteSelectedRows() { const checkedBoxes = document.querySelectorAll('#comp-tbody .row-chk:checked'); if (checkedBoxes.length === 0) return uiNotify("Pilih baris yang akan dihapus!", "danger"); uiConfirmAction("Hapus Baris?", `Yakin menghapus ${checkedBoxes.length} baris?`, () => { checkedBoxes.forEach(box => { box.closest('tr').remove(); }); renumberRows(); editCalc(); editStatus(false); uiNotify("Baris dihapus"); }, true); }

function editSave(silent = false) { 
    const n = storage.find(x => x.id === activeId); const rows = []; document.querySelectorAll('#comp-tbody tr').forEach(tr => { const e = []; tr.querySelectorAll('.col-extra').forEach(inp => e.push(inp.value)); rows.push({ barang: tr.querySelector('.col-barang').value, jml: tr.querySelector('.col-jml').value, harga: tr.querySelector('.col-harga').value, extras: e }); }); 
    ...
    if(!silent) sysAddHistory(`<b>${n.name}</b> <span class="his-act" style="color:var(--edit)">diedit/disimpan</span>`, 'edit'); dbSave(); editStatus(true); if(!silent) uiNotify("Nota Tersimpan!"); 
}
function editCalc() { 
    let t = 0; document.querySelectorAll('#comp-tbody tr').forEach(tr => { const rawQ = sysNum(tr.querySelector('.col-jml').value); const rawP = sysNum(tr.querySelector('.col-harga').value); t += (rawQ * rawP); }); 
    const n = storage.find(i=>i.id===activeId); let finalTotal = t; if (n && n.discVal && n.discVal > 0) { let discountAmount = (n.discType === 'p') ? t * (n.discVal / 100) : n.discVal; if(discountAmount > t) discountAmount = t; finalTotal = t - discountAmount; document.getElementById('txt-old-total').innerText = "Rp " + t.toLocaleString('id-ID'); document.getElementById('txt-old-total').classList.remove('hidden'); } else { document.getElementById('txt-old-total').classList.add('hidden'); } document.getElementById('txt-total').innerText = finalTotal.toLocaleString('id-ID'); 
}
...
// --- FALLBACK COPY UNTUK ANDROID/HTTP ---
function sysCopyHTML() { 
    const h = getFinalHTML(); if(!h) return; 
    if (navigator.clipboard && navigator.clipboard.writeText) { navigator.clipboard.writeText(h).then(() => { uiNotify("Kode HTML Disalin!"); uiPopupClose(); }).catch(err => { fallbackCopy(h); }); } else { fallbackCopy(h); }
}
function fallbackCopy(text) { const textArea = document.createElement("textarea"); textArea.value = text; textArea.style.position = "fixed"; document.body.appendChild(textArea); textArea.focus(); textArea.select(); try { document.execCommand('copy'); uiNotify("Kode HTML Disalin!"); uiPopupClose(); } catch (err) { uiNotify("Gagal menyalin!", "danger"); } document.body.removeChild(textArea); }

function sysDownloadHTMLAction() { if(!document.getElementById('view-editor').classList.contains('hidden')) { editSave(true); } const h = getFinalHTML(); if(!h) return; const n = storage.find(i=>i.id===activeId); const b = new Blob([h], {type: 'text/html'}); const a = document.createElement('a'); a.href = URL.createObjectURL(b); a.download = `${n?n.name:'Nota'}.html`; a.click(); }
...
function uiPreview(show, useStorage = false) {
    ...
    if(show) {
        ...
        let dateHtml = ''; if (n.showDate) { if (n.dateMode === 'double') { dateHtml = `<b>Tgl. Masuk :</b> ${n.date1 ? formatDateIndo(n.date1) : "..."}<br><b>Tgl. Keluar :</b> ${n.date2 ? formatDateIndo(n.date2) : "..."}`; } else { dateHtml = `Tanggal : ${n.date1 ? formatDateIndo(n.date1) : "..."}`; } }
        ...
        let finalTotal = total; let totalHtml = ''; if (n.discVal && n.discVal > 0) { let discountAmount = (n.discType === 'p') ? total * (n.discVal / 100) : n.discVal; if(discountAmount > total) discountAmount = total; finalTotal = total - discountAmount; totalHtml = `<div style="text-align:right; margin-top:20px;"><div style="text-decoration: line-through; color: #64748b; font-size: 14px; margin-bottom: 2px;">Rp ${total.toLocaleString('id-ID')}</div><div class="preview-total" style="font-weight:bold; font-size:22px">Total: Rp ${finalTotal.toLocaleString('id-ID')}</div></div>`; } else { totalHtml = `<div class="preview-total" style="text-align:right; margin-top:20px; font-weight:bold; font-size:22px">Total: Rp ${total.toLocaleString('id-ID')}</div>`; }
...
function sysDownloadImg() { const p = document.getElementById('preview-paper'); const oldTrans = p.style.transform; p.style.transform = 'scale(1)'; p.style.width = '450px'; html2canvas(p, {scale:3}).then(c => { const a = document.createElement('a'); a.download=`Nota.png`; a.href=c.toDataURL(); a.click(); p.style.transform = oldTrans; }); }
...
if (savedNote) { if(savedParent) curParent = savedParent; history.pushState({view: 'folder', id: curParent}, null, ""); editOpen(savedNote); } else if (savedParent) { navGo(savedParent); } else { navRenderGrid(); }
